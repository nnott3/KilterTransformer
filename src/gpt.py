...
# class ClimbingRouteGPT(nn.Module):
...
#         prompt = f"angle{angle_rounded}_grade{grade}_"  # Add underscore to start holds
...
#                 repetition_penalty=1.2,  # Discourage repeating same holds
#             )
...
# def load_model(model_path: str, device: str = "cpu") -> ClimbingRouteGPT:
#     gpt2_model = GPT2LMHeadModel.from_pretrained(model_path)
#     model = ClimbingRouteGPT(vocab_size=gpt2_model.config.vocab_size)
...
#     model = ClimbingRouteGPT(vocab_size=tokenizer.vocab_size)
