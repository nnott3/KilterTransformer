"""
Data preprocessing for Kilter Board routes.
"""
import pandas as pd
import numpy as np
import re
...
class DataPreprocessing:
    """Handle loading, cleaning, and grade conversions for route data."""
    
    def __init__(self, csv_dir: str = "csv_exports"):
        self.csv_dir = csv_dir
        self._grade_dict = None
    
    @property
    def grade_dict(self) -> dict:
        """Lazy load grade dictionary."""
    ...
    def difficulty_to_grades(self, diff_id) -> Tuple[str, str]:
        """Convert difficulty ID to (v_grade, letter_grade)."""
        if pd.isna(diff_id):
            return None, None
        letter_grade, v_grade = self.grade_dict.get(int(diff_id), "/?").split('/')
        return letter_grade, v_grade
    
    def grade_to_difficulty(self, grade: str, grade_type: str = 'v') -> int:
        """Convert V-grade or letter grade to difficulty ID."""
        for diff_id, name in self.grade_dict.items():
            v, letter = name.split('/')
            if grade_type == 'v' and v == grade:
                return diff_id
            elif grade_type == 'letter' and letter == grade:
                return diff_id
        return None
    
    @staticmethod
    def parse_holds(frames_str: str) -> List[Dict[int, int]]:
        """Parse holds from frames string (format: p1145r12p1146r12...)."""
        if pd.isna(frames_str) or frames_str == '':
            return []
        pattern = r'p(\d+)r(\d+)'
        matches = re.findall(pattern, frames_str)
        return [{int(pos): int(role)} for pos, role in matches]
    
    def load_routes(self, cache_path: str = "src/cleaned_routes.csv") -> pd.DataFrame:
        """Load routes from cache or process from CSVs."""
        if os.path.exists(cache_path):
            print(f"Loading cached routes from {cache_path}...")
            routes = pd.read_csv(cache_path)
            routes['holds_data'] = routes['frames'].apply(self.parse_holds)
            return routes
        ...
        routes[['letter_grade', 'v_grade']] = routes['display_difficulty'].apply(
            lambda x: pd.Series(self.difficulty_to_grades(x))
        )
        routes['holds_data'] = routes['frames'].apply(self.parse_holds)
        
        for col in ['angle_y', 'ascensionist_count']:
            routes[col] = routes[col].fillna(0)
            routes[col] = routes[col].replace([np.inf, -np.inf], 0)
            routes[col] = routes[col].astype(int)
        
        
        routes.to_csv('src/routes_df.csv', index=False)
        print(f"Saved to 'src/routes_df.csv'")
        return routes
    
    def clean_routes(self, routes_df: pd.DataFrame, cache_path: str = "src/cleaned_routes.csv") -> pd.DataFrame:
        """Filter to valid commercial Kilter board routes."""
        initial_count = len(routes_df)
        print(f"\nCleaning {initial_count} routes...")
        
        # Apply filters sequentially with counts
        filters = [
            ('min_ascents > 3', routes_df['ascensionist_count'] > 3),
            ('has_difficulty', ~pd.isna(routes_df['display_difficulty'])),
            ('difficulty_range (12-28)', (routes_df['display_difficulty'] > 13) & (routes_df['display_difficulty'] < 29)),
            ('is_listed', routes_df['is_listed'] == 1),
            ('commercial_layout', routes_df['layout_id'] == 1),
            ('has_angle', ~pd.isna(routes_df['angle_y'])),
            ('valid_holds', routes_df['holds_data'].apply(
                lambda holds: all(list(h.keys())[0] in HOLD_ID for h in holds)
            )),
            ('total_holds_count', routes_df['holds_data'].apply(len) < 20),
            
        ]
        
        # Count starts and finishes
        def has_valid_start_finish(holds):
            starts = sum(1 for h in holds if list(h.values())[0] == 12)
            finishes = sum(1 for h in holds if list(h.values())[0] == 14)
            return starts >= 1 and finishes >= 1 and starts <= 2 and finishes <= 2
        
        filters.append(('start/finish count', routes_df['holds_data'].apply(has_valid_start_finish)))
    
        
    
        # Apply all filters and report
        mask = pd.Series([True] * len(routes_df), index=routes_df.index)  # Start with all True
        for name, condition in filters:
            ...
            if dropped > 0:
                print(f"  ✗ {name:25s}: dropped {dropped:5d} ({dropped/before*100:5.1f}%)")
        
        routes_clean = routes_df[mask].copy()
        
        print(f"\n  Final: {len(routes_clean):5d} routes ({len(routes_clean)/initial_count*100:.1f}% retained)")
        
        routes_clean.to_csv(cache_path, index=False)
...
if __name__ == "__main__":
    ...
    cache_path = "src/cleaned_routes.csv"
